...
app_directory_path="$temporary_root/talksphere"

if ! initial_output="$(TALKSPHERE_LOG_LEVEL=warn XDG_DATA_HOME="$temporary_root" "$binary_path" ledger credit_summary 2>&1)"; then
...
if ! summary_output="$(TALKSPHERE_LOG_LEVEL=warn XDG_DATA_HOME="$temporary_root" "$binary_path" ledger credit_summary 2>&1)"; then
